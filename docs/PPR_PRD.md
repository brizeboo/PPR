...
PPR 是一个灵活、轻量且易于集成的报表与数据展示系统。它旨在为现有业务系统提供开箱即用的报表设计、数据查询展示、复杂 Excel 导出功能，而无需业务系统自行开发繁琐的报表模块。系统本身不维护用户体系和角色标识，作为独立的应用运行，通过 Token 透传方式无缝接入主系统权限体系（后台请求主系统接口获取权限字符），极大降低二次开发成本。
...
- **Excel 在线编辑与渲染:** x-spreadsheet (轻量级 Web Excel 组件，用于在浏览器端加载和预览 Excel 模板，提供类似原生 Excel 的可视化网格体验，相比 Luckysheet 能显著减小前端打包体积，支持单元格点击绑定与交互操作)
- **集成与挂载:**
...
- **可视化填空模板设计 (核心特性):**
  ...
  - **拖拽式绑定 (旧版) / 点击式绑定 (新版):** 用户可以在界面中选择已配置好的**查询视图**，系统会列出其可用字段。用户在右侧 Excel 预览器选中单元格后，点击字段旁的 `+` 按钮，将字段插入到单元格中。
  - **自动生成标识符:** 绑定完成后，系统会自动按自定义规则将该单元格替换为对应的底层标识符。支持的三大类填充规则为：
...
- **文件存储与访问:** 提供统一的文件存储目录，支持通过接口将文件上传到系统内，或者直接将文件放置到服务器的指定物理目录下。
- **文件直链访问:** 提供统一的文件访问接口，允许外部系统或前端页面直接通过 URL 访问和预览系统内的静态文件。
- **目录树可视化管理:** 后台管理提供类似 Windows 资源管理器的界面，支持目录的层级管理（新建、重命名、删除）以及目录内文件的管理。
- **文件操作:** 支持在指定目录下进行文件的上传、下载、删除以及获取文件的直接访问 URL，文件列表支持分页展示。
...
- **特性:**
  - 主页面为视图列表（左侧表格）与视图配置区（右侧内容）左右分栏布局。
  - 编辑界面支持在线编写带动态参数的 SQL 语句，设置参数类型及默认值。
  - 支持在界面内进行**实时查询预览与执行结果验证**，点击“运行预览”后将从右侧滑出抽屉展示数据详情与表格。
  - 参数配置支持灵活插入、删除（不再绑定字典，字典功能统一由报表层处理）。
...
- **特性:**
  - 页面采用左右分栏布局。最左侧为“导出模板列表”，中间为 Excel 主设计器及顶部操作栏（支持字典设置、临时数据导出等）。
  - 页面最右侧提供“视图字段面板”，可选择指定的查询视图，并列出其所有的字段与动态参数。
  - 用户在 Excel 预览器中选中目标单元格后，点击右侧字段项的 `+` 按钮，即可将字段插入到当前单元格，系统自动绑定映射关系，实现“所见即所得”的模板变量填充配置。
...
### 4.7 系统日志查看页面 (System Log Viewer Page)

- **功能:** 为管理员提供操作日志、报表访问日志的综合查询面板。
- **特性:** 支持按操作类型、时间区间、状态、耗时及操作人进行组合过滤检索，支持查看异常 SQL 的详细堆栈信息。

### 4.8 文件管理页面 (File Management Page)

- **功能:** 为管理员提供类似 Windows 资源管理器的文件和目录可视化管理界面。
- **特性:**
  - **布局设计:** 采用左右分栏布局。左侧为目录树结构，右侧为主操作区和文件列表。
  - **顶部操作栏:** 右侧上方展示当前访问地址（面包屑或输入框），并提供“新建目录”、“重命名目录”、“删除目录”以及“文件上传”等核心操作按钮。
  - **文件列表:** 右侧下方采用表格形式展示当前选定目录内的文件，支持分页加载。文件操作列包含“下载”、“删除”以及“查看 URL（复制直链）”。

### 4.9 开箱即用的独立管理控制台 (Standalone Admin Console)

- **功能:** 提供一个可直接访问、部署即用的统一管理后台，作为整个 PPR 系统的“报表中心”。
- **集成方式:** 将上述 4.1 ~ 4.8 章节的各个功能页面（数据展示、设计器、权限、数据库、文件管理等），通过 Vue 组件的形式整体拼装在一个标准的中后台布局中（顶部 Header 水平导航菜单 + 下方独立内容区）。
- **特性:**
  - **独立运行:** 若业务系统无需将报表功能深度嵌入到自身页面内，可直接通过浏览器访问该管理页面，以独立子系统的形态运行。
  - **动态菜单路由:** 配合 Sa-Token 鉴权结果（基于主系统返回的权限字符），动态渲染顶部导航菜单，隐藏无权访问的系统配置模块，确保功能安全隔离。
...
- **`GET /api/v1/file/access/**`**：(公开接口) 提供文件的直接访问与下载能力。上传至系统内或放置到指定目录下的文件，均可通过此接口拼接相对路径进行访问。
- **后台文件与目录管理 (`/api/v1/admin/file/*`)**：提供目录层级树查询、新建目录、重命名目录、删除目录功能；提供指定目录下的文件分页列表查询、文件上传、文件下载、文件删除及获取访问 URL 等管理功能。
...
为保证“可嵌入组件产物 + 独立管理控制台 + 后端服务”三者解耦，同时便于后续扩展（多数据源、权限、导出），源码目录采用\*\*前后端分离 + 前端多包（可选）\*\*的组织方式。
