...
## 阶段二点五：M2.5 - 文件管理系统 (File Management)
**目标:** 实现类似 Windows 资源管理器的文件存储、目录树可视化管理、文件上传下载及文件直链对外访问。

### 后端任务 (Backend)
- [ ] **公开文件访问接口**: 提供 `GET /api/v1/file/access/**` 接口，支持文件的直接访问与预览。
- [ ] **后台目录管理接口**: 提供 `/api/v1/admin/file/dir/*` 接口，支持目录层级树查询、新建、重命名、删除目录功能。
- [ ] **后台文件操作接口**: 提供 `/api/v1/admin/file/*` 接口，支持指定目录下的文件分页查询、上传、下载、删除及获取访问 URL 等管理功能。

### 前端任务 (Frontend)
- [ ] **文件管理页面布局与目录树**: 采用左右分栏布局，左侧为目录树结构，展示后台目录层级。
- [ ] **目录操作与顶部工具栏**: 右侧上方展示当前访问地址（面包屑或输入框），并提供“新建目录”、“重命名目录”、“删除目录”以及“文件上传”等核心操作按钮。
- [ ] **文件列表与行级操作**: 右侧下方采用表格形式展示当前选定目录内的文件，支持分页加载。文件操作列包含“下载”、“删除”以及“查看 URL（复制直链）”。
