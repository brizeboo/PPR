...
* **执行步骤**: 
  ...
  2. 配置 Maven 多模块（如 `ppr-core`, `ppr-server`）。
...
* **执行步骤**:
  ...
  5. 开发 `/api/v1/admin/view/preview` 接口返回 `List<Map<String, Object>>`。
* **验收标准**: 
...
### Task F1.3: 视图设计器 - Monaco Editor 集成
* **执行步骤**:
  1. 引入 `monaco-editor` 及其 Vue 封装库（如 `@guolao/vue-monaco-editor`）。
  2. 开发 `/views/view-designer/index.vue`，页面划分为左侧（视图列表）、中上（SQL 编辑区）、中下（参数配置区）、右侧（预览结果区）。
  3. 配置 Monaco 为 `sql` 语言模式，启用基础代码高亮和暗黑主题。
* **验收标准**: 
  * 页面加载后，能在代码编辑器中输入 SQL 并看到明显的语法高亮，编辑器体验流畅。

### Task F1.4: 视图设计器 - 参数绑定与实时预览
* **执行步骤**:
  1. 在视图设计器中开发“参数配置表单”（动态增删行，包含参数名、类型、是否必填）。
  2. 开发“运行”按钮，提取 Monaco 中的 SQL 及配置好的参数，调用后端 `/api/v1/admin/view/preview` 接口。
